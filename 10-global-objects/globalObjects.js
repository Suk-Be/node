...
/*
Node.js global objects are global in nature
and they are available in all modules.

We do not need to include these objects in our application,
rather we can use them directly.
...
// Now call bove function after 2 seconds
setTimeout(printHello, 2000)


/*
clearTimeout(t)
The clearTimeout(t) global function is used to stop a timer
that was previously created with setTimeout().

Here t is the timer returned by the setTimeout() function.
 */
const printHelloIfNotCleared = () => {
  console.log('Hello world will be printed')
}
// Call the function abive after 3 Seconds
const t = setTimeout(printHelloIfNotCleared, 10)

// Now clear the timer immediately (stop a timer that has previously been created with setTimeout)
clearTimeout(t)
...
which can be used to clear the timer using the function clearInterval(t).
 */


const printHelloRepeatedlyWithSetInterval = () => {
  console.log('Hello world repeat every second with setInterval')
}
const s = setInterval(printHelloRepeatedlyWithSetInterval, 1000)

// Now clear the interval immediately (stop a interval that has previously been created with setInterval)
//clearInterval(s)

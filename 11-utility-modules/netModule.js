/*
net module is used to create both servers and clients.
...
net.connect(options[, connectionListener])            - A factory method, which returns a new 'net.Socket'
                                                      and connects to the supplied address and port.

net.connect(port[, host][, connectListener])          - Creates a TCP connection to port on host.
                                                      If host is omitted, 'localhost' will be assumed.
                                                      The connectListener parameter will be added as a listener for the 'connect' event.
                                                      It is a factory method which returns a new 'net.Socket'.

net.connect(path[, connectListener])                  - Creates Unix socket connection to path.
                                                      The connectListener parameter will be added as a listener for the 'connect' event.
                                                      It is a factory method which returns a new 'net.Socket'.

// method net.createConnection() does the same as net.connect()
net.createConnection(options[, connectionListener])   - A factory method, which returns a new 'net.Socket'
                                                      and connects to the supplied address and port.

net.createConnection(port[, host][, connectListener]) - Creates a TCP connection to port on host.
                                                      If host is omitted, 'localhost' will be assumed.
                                                      The connectListener parameter will be added as a listener for the 'connect' event.
                                                      It is a factory method which returns a new 'net.Socket'.

net.createConnection(path[, connectListener])         - Creates Unix socket connection to path.
                                                      The connectListener parameter will be added as a listener for the 'connect' event.
                                                      It is a factory method which returns a new 'net.Socket'.
...
net.isIP(input)                                       - Tests if the input is an IP address.
                                                      ...
                                                      4 for IP version 4 addresses,
                                                      and 6 for IP version 6 addresses.


net.isIPv4(input)                                     - Returns true if the input is a version 4 IP address,
                                                      otherwise returns false.

net.isIPv6(input)                                     - Returns true if the input is a version 6 IP address,
                                                      otherwise returns false.
...
Methods:
server.listen(port[, host][, backlog][, callback])  - Begin accepting connections on the specified port and host.
...
server.listen(handle[, callback])                   - The handle object can be set to either a server or socket
                                                    (anything with an underlying _handle member),
                                                    or a {fd: <n>} object.
                                                    It will cause the server to accept connections on the specified handle,
                                                    but it is presumed that the file descriptor or handle has already been bound to a port or domain socket.
                                                    Listening on a file descriptor is not supported on Windows.

server.listen(options[, callback])                  - The port, host, and backlog properties of options,
                                                    as well as the optional callback function,
                                                    behave as they do on a call to server.listen(port, [host], [backlog], [callback]) .
...
server.unref()                                      - Calling unref on a server will allow the program to exit
...
server.ref()                                        - Opposite of unref,
                                                    calling ref on a previously unrefd server will not let the program exit
                                                    if it's the only server left (the default behavior).
                                                    If the server is refd, then calling the ref again will have no effect.

server.getConnections(callback)                     - Asynchronously get the number of concurrent connections on the server.
...
Events: net.Socket is an eventEmitter and it emits the following events.
...
close           - Emitted once the socket is fully closed.
                The argument had_error is a boolean which indicates if the socket was closed due to a transmission error.


Properties: net.Socket provides many useful properties to get better control over socket interactions.
socket.bufferSize     - This property shows the number of characters currently buffered to be written.

socket.remoteAddress  - The string representation of the remote IP address.
...
socket.remotePort     - The numeric representation of the remote port.
...
socket.localAddress   - The string representation of the local IP address the remote client is connecting on.
                      For example,
                      if you are listening on '0.0.0.0' and the client connects on '192.168.1.1',
...
socket.connect(port[, host][, connectListener]) - Opens the connection for a given socket.
                                                If port and host are given,
                                                then the socket will be opened as a TCP socket,
                                                if host is omitted, localhost will be assumed.
                                                If a path is given, the socket will be opened as a Unix socket to that path.

socket.connect(path[, connectListener])         - Opens the connection for a given socket.
                                                If port and host are given,
                                                then the socket will be opened as a TCP socket,
                                                if host is omitted, localhost will be assumed.
                                                If a path is given, the socket will be opened as a Unix socket to that path.
...
socket.end([data][, encoding])                  - Half-closes the socket, i.e., it sends a FIN packet.
                                                It is possible the server will still send some data.

socket.destroy()                                - Ensures that no more I/O activity happens on this socket.
                                                Necessary only in case of errors (parse error or so).

socket.pause()                                  - Pauses the reading of data.
                                                That is, 'data' events will not be emitted.
                                                Useful to throttle back an upload.

socket.resume()                                 - Resumes reading after a call to pause().

socket.setTimeout(timeout[, callback])          - Sets the socket to timeout after timeout milliseconds of inactivity on the socket.
                                                By default, net.Socket does not have a timeout.

socket.setNoDelay([noDelay])                    - Disables the Nagle algorithm.
                                                By default, TCP connections use the Nagle algorithm,
                                                they buffer data before sending it off.
...
socket.setKeepAlive([enable][, initialDelay])   - Enable/disable keep-alive functionality,
                                                and optionally set the initial delay before the first keepalive probe is sent on an idle socket.
                                                enable defaults to false.


socket.address()                                - Returns the bound address,
                                                the address family name, and the port of the socket as reported by the operating system.
                                                Returns an object with three properties,
                                                e.g. { port: 12346, family: 'IPv4', address: '127.0.0.1' }.

socket.unref()                                  - Calling unref on a socket will allow the program to exit
                                                if this is the only active socket in the event system.
                                                If the socket is already unrefd, then calling unref again will have no effect.


socket.ref()                                    - Opposite of unref,
                                                calling ref on a previously unrefd socket will not let the program exit
                                                if it's the only socket left (the default behavior).
                                                If the socket is refd, then calling ref again will have no effect.

 */

// example can be seen with server.js and client.js in this folder
// How to test:
// first call server.js (watch output)
// second call client.js (watch output)